...
public class DeputadoDao extends ConnectionFactory {
	 ...
	 * @throws SQLException case occurs an error with database.
	 */
	...
	public void adicionaDeputado(ArrayList<Deputados> deputies) throws SQLException {
		String sql;/*Variable that received the code to add deputies on database.*/
		...
		PreparedStatement stmt;/*Variable that create the connection with database.*/
		...
		for( int i = 0; i < sizeDeputies; i++ ) {
			...
			stmt.setString(FIVE, deputies.get(i).getNomeDeTratamentoDoParlamentar());
	...
	public ArrayList<String> getNomesDeputados() throws SQLException {
		...
		while( rs.next() ) {
			...
			String civilName;/*Variable that contains the original name of deputy.*/
			civilName = rs.getString("nomeCivil");
			
			String tratamentName;/*Variable that contains the Treatment Name of deputy.*/
	...
	public ArrayList<Deputados> getDeputados() throws SQLException {
		...
		while( rs.next() ) {
			Deputados deputy;/*Variable that received all features of the deputy.*/
